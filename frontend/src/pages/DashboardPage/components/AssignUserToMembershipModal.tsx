...
export default function AssignUserToMembershipModal({
  open,
  handleClose,
  membershipTypeId,
}: Props) {
  ...
  const assignToMembershipMutation = useMutation(
    ({
      userId,
      membershipTypeId,
    }: {
      ...
      return Api.Memberships.assignMembership(userId, membershipTypeId);
    },
    {
      onError: (error: Error) => {
        ...
        <TableRow key={user.id}>
          ...
          <TableCell>
            <Button
              variant="contained"
              onClick={() =>
                assignToMembershipMutation.mutate({
                  userId: user.id,
                  membershipTypeId,
                })
              }
